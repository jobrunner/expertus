...
**Spec:** `docs/superpowers/specs/2026-09-13-expertus-design.md`
...
- **Basis-URLs stehen nie im Quelltext.** Sie kommen zur Laufzeit aus `/config.json`. Vorgaben: `https://ortus.example`, `https://habitatus.example`, `https://hostus.example`.
...
**habitatus-Deployment.** Zum Planungszeitpunkt antwortete `habitatus.example` nicht. Task 8 testet den Adapter vollständig gegen einen Fake; Task 19 (`make smoke`) ist die Stelle, an der der echte Dienst zum ersten Mal gebraucht wird.
...
|---|---|---|
| ortus | Koordinate → Kopfdaten | `https://ortus.example` |
| hostus | Autosuggest Pflanzennamen | `https://hostus.example` |
| habitatus | Plot → EUNIS-Habitat | `https://habitatus.example` |
...
const BASE = process.env.ORTUS_BASE_URL ?? 'https://ortus.example'
...
```json
{
  "ortusBaseUrl": "https://ortus.example",
  "habitatusBaseUrl": "https://habitatus.example",
  "hostusBaseUrl": "https://hostus.example"
}
```
...
try {
  ...
  window.expertus = { store, actions, hostus, router }
} catch (err) {
...
In `src/app.js` die Platzhalterzeile `window.expertus = …` ersetzen durch:
...
- Produces: Image `expertus:dev`, das auf `:8080` ausliefert und `ORTUS_BASE_URL`, `HABITATUS_BASE_URL`, `HOSTUS_BASE_URL` auswertet
...
ENV ORTUS_BASE_URL=https://ortus.example \
    HABITATUS_BASE_URL=https://habitatus.example \
    HOSTUS_BASE_URL=https://hostus.example \
    NGINX_PORT=8080
...
const ORTUS = process.env.ORTUS_BASE_URL ?? 'https://ortus.example'
const HOSTUS = process.env.HOSTUS_BASE_URL ?? 'https://hostus.example'
const HABITATUS = process.env.HABITATUS_BASE_URL ?? 'https://habitatus.example'
